...
public class LibraryDB {
    static final Logger log = LogManager.getLogger(LibraryDB.class.getName());
    ...
    public void addReader(String name) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            if (recCnt > 0) {
                log.info("Читатель '{}' добавлен", name);
            } else {
                log.error("Читатель '{}' не был добавлен.", name);
            }
        } catch (SQLException e) {
            log.error(e.fillInStackTrace());
        }
    ...
    public void takeBook(int bookNum, int readerNum) {
        ...
        if (!isBookAvailable(bookNum)) {
            log.error("Книга №{} недоступна в библиотеке.", bookNum);
            allRight = false;
        }

        if (!isReaderFound(readerNum)) {
            log.error("Читатель №{} не найден.", readerNum);
            allRight = false;
        }

        if (isAlreadyTaken(bookNum, readerNum)) {
            log.error("Книга №{} уже назначена на читателя №{}.", bookNum, readerNum);
            allRight = false;
        }
        ...
        if (allRight) {
            ...
            if (delivered) {
                ...
                try (PreparedStatement ps = con.prepareStatement(sqlBook)) {
                    ...
                    if (recCnt > 0) {
                        log.info("Книга №{} назначена читателю №{}.", bookNum, readerNum);
                    } else {
                        log.error("Книга №{} не была назначена читателю №{}.", bookNum, readerNum);
                    }
                } catch (SQLException e) {
                    log.error(e.fillInStackTrace());
                }
    ...
    public void returnBook(int bookNum, int readerNum) {
        ...
        if (!isAlreadyTaken(bookNum, readerNum)) {
            log.error("Не надено книги №{} назначенной на читателя №{}.", bookNum, readerNum);
            allRight = false;
        }
        ...
        if (allRight) {
            ...
            if (delivered) {
                ...
                try (PreparedStatement ps = con.prepareStatement(sqlBook)) {
                    ...
                    if (recCnt > 0) {
                        log.info("Книга №{} снята с читателя №{}.", bookNum, readerNum);
                    } else {
                        log.error("Книга №{} не была снята с читателя №{}.", bookNum, readerNum);
                    }
                } catch (SQLException e) {
                    log.error(e.fillInStackTrace());
                }
    ...
    public void getStatistics() {
        ...
        try (Statement st = con.createStatement();
            ...
            log.info("");
            log.info("Статистика по авторам:");
    ...
    public void searchByAuthor(String authorName) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE,
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                if (recCnt == 0) {
                    log.error("Книги автора '{}' не найдены.", authorName);
                } else {
                    printBooksString("№", "Автор", "Название", "Год(ы)", "Кол-во");
                    printBooksString("====", "=====", "========", "======", "======");
    ...
    public void searchByTitle(String bookTitle) {
        ...
        try (PreparedStatement ps = con.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE,
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                if (recCnt == 0) {
                    log.error("Книги c названием '{}' не найдены.", bookTitle);
                } else {
                    printBooksString("№", "Автор", "Название", "Год(ы)", "Кол-во");
                    printBooksString("====", "=====", "========", "======", "======");
